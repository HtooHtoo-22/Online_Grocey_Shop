...
<c:forEach items="${SearchedProducts}" var="search">
                ...
                <a href="#" onclick="showModal('${search.id}', '${search.productName}', '${search.description}', 'data:image/jpeg;base64,${search.getBase64()}', ${search.price}, ${search.per_quantity}, '${search.unit}','${search.categoryName}','${search.quantity}')">
                    <img src="data:image/jpeg;base64,${search.getBase64()}" class="product-img" alt="${search.productName}"/>
                </a>
                <div>
                    <div>${search.productName}</div>
                    <div>${search.price} KS</div>
                    <div>${search.per_quantity} ${search.unit}</div>
                   <button type="button" class="add-to-cart-button" onclick="addToCart('${search.id}', '${search.productName}', ${search.price}, '${search.getBase64()}','${search.quantity}','${search.per_quantity}')">Add to Cart</button>
                   <button type="button" class="favorite-button" data-product-id="${search.productId}" onclick="toggleFavorite(this)">Favorite</button>
